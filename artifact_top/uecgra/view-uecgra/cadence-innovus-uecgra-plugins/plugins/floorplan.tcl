...
set tile_width  [expr 75]
set tile_height [expr 75]
set inter_tile_margin_w [expr 3]
set inter_tile_margin_h [expr 3]
...
for {set i 1} {$i < 32} {incr i} {
  ...
  # clksel
  set clksels [dbGet top.terms.name clksel__${i}__*]
  for {set clksel_i 0} {$clksel_i < [llength $clksels]} {incr clksel_i} {
    set bottom_pins \
        [linsert $bottom_pins $num_ports [lindex $clksels $clksel_i]]
  }
}
...
# clk_out_s
set clk_outs [dbGet top.terms.name clk_out_s*]
for {set clk_out_i 0} {$clk_out_i < [llength $clk_outs]} {incr clk_out_i} {
  set bottom_pins \
      [linsert $bottom_pins $num_ports [lindex $clk_outs $clk_out_i]]
}
...
for {set i 33} {$i < 64} {incr i} {
  ...
  for {set clksel_i 0} {$clksel_i < [llength $clksels]} {incr clksel_i} {
    set top_pins \
        [linsert $top_pins $num_ports [lindex $clksels $clksel_i]]
  }
}
...
# clk_out_n
set clk_outs [dbGet top.terms.name clk_out_n*]
for {set clk_out_i 0} {$clk_out_i < [llength $clk_outs]} {incr clk_out_i} {
  set top_pins \
      [linsert $top_pins $num_ports [lindex $clk_outs $clk_out_i]]
}
...
# clk_out_w
set clk_outs [dbGet top.terms.name clk_out_w*]
for {set clk_out_i 0} {$clk_out_i < [llength $clk_outs]} {incr clk_out_i} {
  set left_pins \
      [linsert $left_pins $num_ports [lindex $clk_outs $clk_out_i]]
}
...
# clk_out_e
set clk_outs [dbGet top.terms.name clk_out_e*]
for {set clk_out_i 0} {$clk_out_i < [llength $clk_outs]} {incr clk_out_i} {
  set right_pins \
      [linsert $right_pins $num_ports [lindex $clk_outs $clk_out_i]]
}
