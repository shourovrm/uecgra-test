...
def plot_stack( jpath={} ):
  ...
  bar_names  = [ 'Ultra-Elastic Tile', '' ]
  mul_area   = [ data['mul'     ]['abs_total'], 0.0 ]
  alu_area   = [ data['alu'     ]['abs_total'], 0.0 ]
  acc_area   = [ data['acc_reg' ]['abs_total'], 0.0 ]
  q_n_area   = [ data['in_qs__0']['abs_total'], 0.0 ]
  q_s_area   = [ data['in_qs__1']['abs_total'], 0.0 ]
  q_w_area   = [ data['in_qs__2']['abs_total'], 0.0 ]
  q_e_area   = [ data['in_qs__3']['abs_total'], 0.0 ]

  sup_n_area = data['suppressors__0']['abs_total']
  sup_s_area = data['suppressors__1']['abs_total']
  sup_w_area = data['suppressors__2']['abs_total']
  sup_e_area = data['suppressors__3']['abs_total']
  sup_area   = [ sup_n_area+sup_s_area+sup_w_area+sup_e_area, 0.0 ]

  unsafe_gen_area = [ data['unsafe_gen']['abs_total'], 0.0 ]
  clk_switcher_area = [ data['clk_switcher']['abs_total'], 0.0 ]
  ...
  sup_area        = np.array( sup_area   )
  unsafe_gen_area = np.array( unsafe_gen_area )
  clk_switcher_area = np.array( clk_switcher_area )
  ...
  bottom_sup = bottom_q_e + q_e_area
  bar_sup = plt.bar( ind, sup_area, width, bottom=bottom_sup,
      color=cdict['suppress'], edgecolor=cdict['suppress'])

  bottom_unsafe_gen = bottom_sup + sup_area
  bar_unsafe_gen = plt.bar( ind, unsafe_gen_area, width, bottom=bottom_unsafe_gen,
      color=cdict['unsafe_gen'], edgecolor=cdict['unsafe_gen'])

  bottom_clk_switcher = bottom_unsafe_gen + unsafe_gen_area
  bar_clk_switcher = plt.bar( ind, clk_switcher_area, width, bottom=bottom_clk_switcher,
      color=cdict['clk_switcher'], edgecolor=cdict['clk_switcher'])

  bottom_other = bottom_clk_switcher + clk_switcher_area
  ...
  plt.legend(
    ( bar_mul[0], bar_alu[0], bar_acc[0], bar_q_n[0], bar_q_s[0], bar_q_w[0], bar_q_e[0], bar_sup[0],  bar_unsafe_gen[0], bar_clk_switcher[0], bar_other[0] )[::-1],
    ( 'mul',      'alu',      'acc',      'in_q_n',      'in_q_s',      'in_q_w',      'in_q_e',      'suppress',  'unsafe_gen', 'clk_switcher',     'other'      )[::-1]
  )
